...
(function () {
  ...
  const I18N = {
    ...
    ru: {
      ...
      'gift.d': 'Подарочная карта AlesSanna — заботливый подарок на любой повод.',
      ...
      'form.ok': 'Спасибо! Откроем письмо — добавьте сообщение и отправьте.',
  ...
  let current = localStorage.getItem('as-lang') || (navigator.language || 'et').slice(0, 2).toLowerCase();
  ...
  const toggleMenu = (force) => { const open = force != null ? force : !menu.classList.contains('is-open'); menu.classList.toggle('is-open', open); burger.classList.toggle('is-open', open); burger.setAttribute('aria-expanded', String(open)); menu.setAttribute('aria-hidden', String(!open)); document.body.style.overflow = open ? 'hidden' : ''; };
  burger.addEventListener('click', () => toggleMenu());
  menu.querySelectorAll('a').forEach((a) => a.addEventListener('click', () => toggleMenu(false)));
  const io = new IntersectionObserver((entries) => { entries.forEach((e) => { if (e.isIntersecting) { e.target.classList.add('is-visible'); io.unobserve(e.target); } }); }, { threshold: 0.14, rootMargin: '0px 0px -40px 0px' });
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (!name || !phone) { status.textContent = I18N[current]['form.err']; return; }
