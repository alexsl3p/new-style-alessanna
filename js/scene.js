/* ============================================================
   AlesSanna Ilusalong — Three.js background
   A calm, flowing domain-warped gradient (fbm noise) in warm
   nude / champagne / rose tones, plus drifting light particles
   and gentle mouse parallax. Falls back to CSS when WebGL or
...
// Bail out gracefully — the CSS gradient (body::before) stays visible.
function bail() {
...
function init() {
  ...
  // ---- Flowing gradient plane (full-screen quad) ----
  const uniforms = {
    u_time:   { value: 0 },
    u_res:    { value: new THREE.Vector2(window.innerWidth, window.innerHeight) },
    u_mouse:  { value: new THREE.Vector2(0.5, 0.5) },
    u_cA:     { value: new THREE.Color(0x14100c) }, // deep mocha
    u_cB:     { value: new THREE.Color(0x7a5a33) }, // warm bronze
    u_cC:     { value: new THREE.Color(0xc9a36a) }, // champagne gold
    u_cD:     { value: new THREE.Color(0xb98a7e) }, // soft rose
  };
  ...
  const fragmentShader = /* glsl */`
    ...
    void main() {
      ...
      // Domain warping for soft, silky flow
      ...
      vec2 m = (u_mouse - 0.5);
      f += 0.06 * sin((uv.x + m.x) * 6.2831 + t * 2.0);

      // Build the warm palette
      ...
      // Soft vignette toward the deep mocha base
  ...
  const quad = new THREE.Mesh(
    ...
    new THREE.ShaderMaterial({ uniforms, vertexShader, fragmentShader, depthTest: false })
  );
  scene.add(quad);

  // ---- Drifting light particles ("dust in the light") ----
  ...
  const sprite = (() => {
    ...
    grad.addColorStop(0.3, 'rgba(226,199,154,0.7)');
    grad.addColorStop(1, 'rgba(226,199,154,0)');
    g.fillStyle = grad;
  ...
  function animate() {
    ...
    // drift particles upward, wrap around, gentle sway
    ...
    points.position.x = (mouse.x - 0.5) * 0.08;
    points.position.y = (mouse.y - 0.5) * 0.08;

    renderer.render(scene, camera);

    if (!started) { started = true; finishLoader(); }
  }
  animate();
}